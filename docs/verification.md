...
本文保留 2026-09-08 的首版验收快照。2026-09-09 对“是否需要、如何找到、是否实际
使用”的增量实现和新检查见 [路由验证](routing-validation.md)。
...
| 验收 | 实际结果与证据 | 验证范围 |
| --- | --- | --- |
| E01 单入口 | 只指定 SkillNav，生成 [报告](examples/handoff/item-report.md)，3 行、49.65；安装后新 CLI 入口另生成 45.10 报告 | 隔离文件激活及真实用户目录 CLI 入口通过；桌面菜单未单独查看 |
...
再只移除该临时副本并核验不存在。随后用户明确确认真实目录安装，结果见下文。
...
复制成功不等于当前 Codex 会话已发现，因此安装后另做了新会话显式引用验收。

2026-09-08 用户确认后，安装到 `~/.agents/skills/skillnav`，没有覆盖已有目录。
10 个文件逐项哈希与本地提交 `d6d3e77` 的技能代码一致，已安装扫描脚本严格检查通过。
启动新的 Codex CLI ephemeral 会话，提示仅指定 `$skillnav`，不提供入口文件路径；
真实命令回执显示读取用户安装目录中的 SKILL.md，自动执行两个测试脚本并生成
[report.md](examples/installed-entry/report.md)。主任务独立复核 3 行、逐项
12.15／22.20／10.75、合计 45.10，输入和来源文件保留，安装文件哈希复核一致。
本次 SkillNav 记忆不读不写。见 [安装及入口检查](evidence/user-install-entry.json)。

宿主提示技能清单超出上下文预算；本次显式入口仍成功，不据此推断所有技能可见。
尝试连接现有 app-server 代理未建立连接，因此未取得桌面菜单刷新证据；没有为此
修改配置、启动守护进程或重启用户应用。上述通过结论限定于实际测试的 CLI 入口。
...
本地安装验证后，用户明确批准测试通过便推送 GitHub；推送目标为
`feat/skillnav-v0.1.0`，实际远端内容以该分支为准。本任务未合并、未改可见性、
未创建 Release。真实用户自助安装、真实任务、跨平台、真实远端副作用
