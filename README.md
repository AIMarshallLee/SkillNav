...
内置本地搜索、基础静态风险检查、创建与结构校验、输入与产物检查脚本，以及相应工作流程。
...
最新预检与交付物验收见 [本轮验证](docs/p0-validation.md)，
单入口集成见 [能力验证](docs/bundle-validation.md)；
...
静态审查、文件契约、状态与运行记录辅助代码只用标准库。当前已测环境为 macOS arm64、Python 3.11.9、
...
1. 检查材料与目标。简单任务直接完成；已指定审查对象或创建规则时直接进入对应流程。
2. 需要时从已授权目录检索；目录截断或描述不全时，按任务词细化或翻页。
3. 选定流程前核对输入格式、工具、依赖和预期产物，保留已验证、缺失与未知状态；检查调用策略及相关风险。
4. 实际执行并核对产物，再交给下一步。实际文件与说明报告分别验收；创建任务生成草稿并进行真实试用。
...
多文件交接和试用可使用内置[文件契约检查](skills/skillnav/references/execution.md#preflight-and-file-contracts)：
执行前核对输入与宿主提供的工具/依赖观察，执行后逐个读取交付物并检查内容。
普通用户无需填写配置。检查器不能认证宿主观察的真假，列数、标题和哈希也不代表
金额、事实或全部语义都正确；实际任务仍需对应的独立核对。
