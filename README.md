...
**只调用 SkillNav，剩下的技能让它来选、来用、来检查。**

你已经装了很多 Skill，却未必记得哪个适合当前任务。SkillNav 在 Codex
...
1. 形成最小任务约定：目标、输入、产物、不变项、检查和权限。
2. 从真实可用候选选择通常 1–3 个技能；简单工作可以直接完成。
3. 读取入选技能完整指令并实际执行，检查后向下游交接。
4. 失败先诊断；每步最多两次额外尝试、全任务最多两次换路，无新证据则更早停止。
5. 交付可检查产物。经许可记录最小结果；用户未反馈始终不记为接受。
